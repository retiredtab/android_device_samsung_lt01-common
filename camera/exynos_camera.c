 ...
 * Modified for TAB 3 8.0 by Toha <tohenk@yahoo.com>
 *
...
void exynos_camera_put_parameters(struct camera_device *dev, char *params)
{
	...
	exynos_camera = (struct exynos_camera *) dev->priv;
	ret = exynos_params_string_set(exynos_camera, params);
	if (!ret)
		ALOGE("%s: Couldn't put parameters", __func__);
...
struct camera_module HAL_MODULE_INFO_SYM = {
	.common = {
		...
		.name = "LT01 Exynos Camera",
		.author = "Paul Kocialkowski, Scott Brissenden, Toha",
